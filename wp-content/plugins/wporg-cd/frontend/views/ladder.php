...
function wporgcd_render_ladder_view($filters) {
    ...
    foreach ($ladder_stats as $s) {
        ...
        if (!empty($s['ids']['active']))   { $all_active_ids   = array_merge($all_active_ids,   $s['ids']['active']); }
        if (!empty($s['ids']['warning']))  { $all_warning_ids  = array_merge($all_warning_ids,  $s['ids']['warning']); }
        if (!empty($s['ids']['inactive'])) { $all_inactive_ids = array_merge($all_inactive_ids, $s['ids']['inactive']); }
    }
        ...
        <div class="card">
            ...
            <div class="funnel">
                ...
                foreach ($lids as $i => $lid):
                    $l = $ladders[$lid];
                    $s = $ladder_stats[$lid] ?? array('count' => 0, 'active_count' => 0, 'warning_count' => 0, 'inactive_count' => 0);
                    $cnt = $s['count'];
                    $w = $bar_width($cnt);
                ?>
